...
void generar_dot(int** traceback, const char* cad1, const char* cad2, const string nombre_archivo){
    int columna = strlen(cad1);
    int fila = strlen(cad2);
    ...
    if (archivo.is_open()) {
        ...
        // Dibujar la matriz de puntuación
        for (int i = 0; i < fila; ++i) {
            for (int j = 0; j < columna; ++j) {
                string nodeName = "node_" + to_string(i) + "_" + to_string(j);
                archivo << nodeName << " [label=\"" << traceback[i][j] << "\"];\n";
                
                // Conectar nodos adyacentes (dependiendo de las direcciones)
                if (i + 1 < fila) archivo << nodeName << " -> node_" << i + 1 << "_" << j << ";\n";
                if (j + 1 < columna) archivo << nodeName << " -> node_" << i << "_" << j + 1 << ";\n";
                if (i + 1 < fila && j + 1 < columna) archivo << nodeName << " -> node_" << i + 1 << "_" << j + 1 << ";\n";
            }
        }

        // Resaltar el camino de alineación óptimo
        int i = fila - 1;
        int j = columna - 1;
        while (i > 0 || j > 0) {
            string nodeName = "node_" + to_string(i) + "_" + to_string(j);
            if (traceback[i][j] == 1) {
                archivo << nodeName << " [color=red, style=filled, fillcolor=yellow];\n";  // Resaltar nodo
            }
            
            // Seguir el camino de traceback (mover según los valores de la matriz)
            if (i > 0 && j > 0 && traceback[i][j] == 1) {  // Diagonal
                i--;
                j--;
            } else if (i > 0 && traceback[i][j] == 2) {  // Arriba
                i--;
            } else if (j > 0 && traceback[i][j] == 3) {  // Izquierda
                j--;
            }
        }
        ...
        archivo << "}\n";
        archivo.close();
        cout << "El archivo .dot ha sido generado correctamente en " << nombre_archivo << endl;
    } else {
